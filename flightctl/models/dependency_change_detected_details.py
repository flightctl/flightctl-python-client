...
class DependencyChangeDetectedDetails(BaseModel):
    """
    ...
    resource_key: StrictStr = Field(description="The resource key identifying the dependency that changed (e.g. \"git:my-repo/main\").", alias="resourceKey")
    fingerprint: StrictStr = Field(description="The new fingerprint (e.g. commit SHA) of the changed dependency.")
    __properties: ClassVar[List[str]] = ["detailType", "resourceKey", "fingerprint"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DependencyChangeDetected']):
            raise ValueError("must be one of enum values ('DependencyChangeDetected')")
        return value
