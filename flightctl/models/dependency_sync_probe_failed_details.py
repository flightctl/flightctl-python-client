...
class DependencySyncProbeFailedDetails(BaseModel):
    """
    ...
    resource_key: StrictStr = Field(description="The resource key identifying the dependency that failed (e.g. \"git:my-repo/main\").", alias="resourceKey")
    error: StrictStr = Field(description="The error message from the failed probe.")
    __properties: ClassVar[List[str]] = ["detailType", "resourceKey", "error"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DependencySyncProbeFailed']):
            raise ValueError("must be one of enum values ('DependencySyncProbeFailed')")
        return value
